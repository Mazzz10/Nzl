...
export default function SearchResults({ params, navigateTo }: { params: SearchParams; navigateTo: ReturnType<typeof useAppState>['navigateTo'] }) {
  ...
  const checkInFormatted = params.checkIn ? format(new Date(params.checkIn), 'MMM d') : '';
  const checkOutFormatted = params.checkOut ? format(new Date(params.checkOut), 'MMM d') : '';
  const searchSummary = `${params.destination || 'Anywhere'} ${checkInFormatted && checkOutFormatted ? `· ${checkInFormatted}–${checkOutFormatted}` : ''} · ${params.guests} Guest${params.guests > 1 ? 's' : ''}`;
    ...
    <div className="flex flex-col min-h-screen bg-muted/20">
      ...
      <div className="bg-card border-b border-border sticky top-16 z-40">
        <div className="container mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="text-sm font-medium">{searchSummary}</div>
      ...
      <div className="container mx-auto px-4 py-8 flex-1">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:hidden flex justify-between items-center mb-4">
            <h1 className="text-2xl font-serif font-semibold">{sampleHotels.length} properties found</h1>
          ...
          <div className={`lg:col-span-3 space-y-6 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="bg-card p-6 rounded-xl border border-border shadow-sm sticky top-40">
              ...
              {/* Price Filter */}
              <div className="mb-8">
                <h3 className="text-sm font-medium mb-4">Price per night</h3>
                <Slider 
              ...
              {/* Star Rating Filter */}
              <div className="mb-8">
                <h3 className="text-sm font-medium mb-4">Star Rating</h3>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {['Pool', 'Spa', 'Gym', 'Free WiFi', 'Pet Friendly', 'Restaurant'].map(amenity => (
